...
} = require("./study-server-routes");
const { registerAdminAuth } = require("./admin-auth");
const {
  configureRateLimitProxy,
  createParticipantRateLimits
} = require("./participant-rate-limits");
...
const REPO_ROOT = fs.existsSync(path.join(__dirname, "configs", "comparison_survey.json"))
  ? __dirname
  : path.resolve(__dirname, "..");
const STIMULI_ROOT = path.join(REPO_ROOT, "stimuli_pipe");
const { getPublicAppConfig } = require("./public-app-config");
...
app.use("/models", express.static(path.join(REPO_ROOT, "models")));
app.use("/vendor/jspsych", express.static(path.join(__dirname, "node_modules", "jspsych")));
app.use("/vendor/@jspsych", express.static(path.join(__dirname, "node_modules", "@jspsych")));

const adminAuth = registerAdminAuth(app, { publicDir: PUBLIC_DIR });
const participantLimits = createParticipantRateLimits();

configureRateLimitProxy(app);

registerStudyPageRoutes(app, PUBLIC_DIR, {
  adminPageHandler: adminAuth.adminPageHandler,
  studyPageLimiter: participantLimits.studyPages
});
registerComparisonApiRoutes(app, REPO_ROOT, {
  requireAdmin: adminAuth.requireAdmin,
  comparisonTrialsLimiter: participantLimits.comparisonTrials
});
registerCustomStudyApiRoutes(app, REPO_ROOT, { requireAdmin: adminAuth.requireAdmin });
...
app.post("/api/log", participantLimits.trialLog, async (req, res) => {
  try {
